    ...
    while True:
        try:
            ...
            if "BANNED" in str(req_game.text) or req_game.status_code == 403:
                ...
                print(f"{red}[{white}{logTime()}{red}] {yan}Nickname: {green}{req_game.json()['User']['Username']} {yellow}| Country: {green}{req_game.json()['User']['Country']} {red}| Trophy: {green}{req_game.json()['User']['SkillRating']} {white}| Crown: {green}{req_game.json()['User']['Crowns']}")
        except:
            continue
